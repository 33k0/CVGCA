...
const { Photostack } = require("../models");
...
router.get("/", async (req, res) => {
  try {
    ...
    const rows = await Photostack.findAll({
      limit,
      offset,
    ...
    console.error("GET /Photostack error:", err);
...
router.get("/:id", async (req, res) => {
  try {
    const row = await Photostack.findByPk(req.params.id);
    if (!row) return res.status(404).json({ error: "Oldhighlight not found." });
    res.json(row);
  } catch (err) {
    console.error("GET /Photostack/:id error:", err);
...
router.post("/", async (req, res) => {
  try {
    ...
    const created = await Photostack.create({ image: req.body.image.trim() });
    res.status(201).json(created);
  } catch (err) {
    console.error("POST /Photostack error:", err);
...
router.put("/:id", async (req, res) => {
  try {
    ...
    console.error("PUT /Photostack/:id error:", err);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.error("DELETE /Photostack/:id error:", err);
